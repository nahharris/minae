...
func (w *World) GetBlock(x, y, z int) BlockType {
	chunkX := int(math.Floor(float64(x) / float64(ChunkWidth)))
	chunkZ := int(math.Floor(float64(z) / float64(ChunkWidth)))
	
	localX := x - chunkX*ChunkWidth
	localZ := z - chunkZ*ChunkWidth
