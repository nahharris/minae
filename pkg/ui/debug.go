...
func (u *UIManager) updateDebugStrings() {
	...
	if u.Player != nil {
		...
		currentXYZ := [3]float32{pos.X, pos.Y, pos.Z}
		if currentXYZ != u.cachedXYZ {
			u.cachedXYZ = currentXYZ
			u.debugXYZ = fmt.Sprintf("XYZ: %.2f / %.2f / %.2f", pos.X, pos.Y, pos.Z)
		}
	} else if u.debugXYZ != "" {
		u.debugXYZ = ""
		u.cachedXYZ = [3]float32{}
	}
	...
	if u.Player != nil {
		...
		currentChunk := [4]int{chunkX, chunkZ, localX, localZ}
		if currentChunk != u.cachedChunk {
			u.cachedChunk = currentChunk
			u.debugChunk = fmt.Sprintf("Chunk: %d, %d (Loc: %d, %d)", chunkX, chunkZ, localX, localZ)
		}
	} else if u.debugChunk != "" {
		u.debugChunk = ""
		u.cachedChunk = [4]int{}
	}
	...
	if u.Player != nil {
		...
		currentDir := [3]float32{dir.X, dir.Y, dir.Z}
		if currentDir != u.cachedDir {
			u.cachedDir = currentDir
			u.debugDir = fmt.Sprintf("Dir: %.2f, %.2f, %.2f", dir.X, dir.Y, dir.Z)
		}
	} else if u.debugDir != "" {
		u.debugDir = ""
		u.cachedDir = [3]float32{}
	}
}
